...
create_h_border_groups <- function(df,formats){
  ...
  borders_df <- 
    df %>% 
    mutate(right_border =  local_format_id %>% map_lgl(~formats$local$border$right$style[[.x]] != "")) %>%
    mutate(left_border = local_format_id %>% map_lgl(~formats$local$border$left$style[[.x]] != "")) %>% 
    select(address,row_temp,col,left_border,right_border) 
  
  left_borders <- borders_df$col[borders_df$left_border] %>% .[!is.na(.)] %>% unique 
  right_borders <- borders_df$col[borders_df$right_border] %>% .[!is.na(.)] %>% unique
  
  right_border <- c(right_borders,left_borders - 1) %>% unique %>% sort()
  
  current_row <- df$row_temp %>%   unique
  
  borders_df <- 
    tibble(max_col = right_border) %>% 
...
create_v_border_groups <- function(df,formats){
  ...
  bottom_borders <- borders_df$row[borders_df$bottom_border] %>% .[!is.na(.)] %>% unique
  
  bottom_borders
  
  border <- c(top_borders,bottom_borders + 1) %>% unique %>% sort()
