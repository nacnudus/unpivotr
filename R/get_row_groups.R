...
get_row_groups <- function(sheet, value_ref, formats, .groupings = groupings(fmt_alignment_indent),
  ...
  header_df <- header_df %>% dplyr::mutate(col_temp = col)
  ...
  header_df <-
    ...
    dplyr::mutate(header_label = paste0("row_header_label_", stringr::str_pad(row_no_name, 2, side = "left", "0")))
  ...
  empty_row_df <-
    ...
    dplyr::mutate(empty_share = ifelse(is.na(empty_share) | empty_share == 1, 1, 0))
  ...
  header_df <-
    header_df %>%
    dplyr::mutate(direction = ifelse(wnw_vector, "WNW", default_row_header_direction)) %>%
...
get_row_sum <- function(data, sheet) {
  data %>%
    dplyr::mutate(row_sum_values = purrr::map_dbl(
      row,
      function(x) {
