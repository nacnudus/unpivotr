...
tables with multi-level column or row headers, or spreadsheets, or nested HTML
tables.  Header and data cells can selected by their contents, position and
...
```{r, echo = TRUE}
cells <- tidy_table(pivoted, col_names = FALSE)
...
unpivoted <-
  data_cells %>%
  enhead(col_headers$`1`, "NNW") %>% # Search north (up) and north-west (up-left)
  enhead(col_headers$`2`, "N") %>% # Search directly north (up)
  enhead(row_headers$`1`, "WNW") %>% # Search west (left) and north-west
  enhead(row_headers$`2`, "W") # Search directly left (west)
unpivoted
```
