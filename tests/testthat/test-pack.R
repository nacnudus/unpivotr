...
test_that("pack() works on common data types", {
  ...
  expect_equal(unname(purrr::map_chr(y$value, pillar::type_sum)),
               c(rep(c("lgl", "int", "dbl", "cpl", "date",
                       "dttm", "chr", "fct", "ord"),
                     each = 2),
                 "int", "chr"))
  expect_equal(names(y$value),
               rep(c("lgl", "int", "dbl", "cpl", "date",
...
test_that("unpack() orders type columns alphabetically", {
  y <- pack(x)
  z <- unpack(y)
  expect_equal(colnames(z),
               c("row", "col", "data_type", "chr", "cpl", "date",
