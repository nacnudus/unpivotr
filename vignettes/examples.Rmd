...
unpivotr_example("worked-examples.xlsx") %>% 
 xlsx_cells_fmt(sheets = "pivot-notes") %>% 
  ...
  locate_if(data_type == "character" & fmt_font_color_rgb !="FFFF0000", direction = "WNW",name = subject_type) %>%
  locate(direction = "WSW",name = subject_comment) %>%
  locate_if(direction = "W",name = subject) %>% 
  locate_if(fmt_font_italic == FALSE, direction = "NNW",name = gender) %>% 
  locate_if(fmt_font_italic == TRUE, direction = "NNE",name = note) %>% 
  locate_if(fmt_font_italic == FALSE,  direction = "N",name = name) %>% 
  migrate()
