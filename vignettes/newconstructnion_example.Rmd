...
testsheet_df <- xlsx_cells_fmt(path = unpivotr_example("newresconst.xlsx"), sheets = "Table 1 - Permits")
# formats <- xlsx_formats_fmt(unpivotr_example("newresconst.xlsx"))

min_row <-  testsheet_df %>% filter(str_detect(character,"RSE")) %>% pull(row) %>% min

testsheet_df %>% filter(row < min_row) %>% 
  locate_data(!is.na(numeric) & col > 1) %>% 
  locate_header_groups(type = "row", .groupings = groupings(!is.na(numeric),str_detect(character, "\\.") )) %>%  
  locate_header_groups(type = "col", header_fill = "style") %>% 
  locate_header_groups(type = "meta", headerheader_fill = "none") %T>% 
  plot_cells(interactive = FALSE,text = local_format_id) %>% 
  migrate()
