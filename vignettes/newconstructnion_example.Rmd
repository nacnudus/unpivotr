...
testsheet_df %>% 
  orientate_auto(formats = formats, manual_value_references = "B8:M23",
...
cnr_df <- testsheet_df$character %>% str_detect("Average RSE") %>% which() %>% 
          min() %>%  testsheet_df[.,c("row","col")]
...
# Clean up 
housing_df <- 
  housing_df %>% set_names(c("row","col",".value","region","units","year_var","month_var")) %>% 
...
all_sheets_housing %>% filter(units %in% c("Total","1 unit" )) %>% 
