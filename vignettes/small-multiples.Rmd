...
unpivoted <-
  purrr::map_dfr(partitions$cells,
                 ~ .x %>%
                   ...
                   behead("NNW", "price") %>%
                   behead("N", "bid_offer")) %>%
  dplyr::select(-data_type, -character, -date)
unpivoted
```
...
row_headers <-
  ...
  dplyr::mutate(character = ifelse(!is.na(character),
                            character,
                            format(date, origin="1899-12-30", "%b-%y"))) %>%
  dplyr::select(row, col, character) %>%
  nest(-row) %>%
  dplyr::mutate(row_header = map(data,
                          ~ str_trim(paste(.x$character, collapse = " ")))) %>%
  unnest(row_header) %>%
  dplyr::mutate(col = 2L) %>%
  dplyr::select(row, row_header)
