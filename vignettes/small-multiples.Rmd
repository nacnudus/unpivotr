...
```{r}
path <- system.file("extdata/frank_ermis__11178__Enron Pricing Report.xlsx",
                    package = "unpivotr")
```
...
Spreadsheet cells are imported with the `contents()` function.  By default, this
returns all the sheets in the spreadsheet, as elements in a list.  Since there
is only one sheet in this file, it is a list of length 1, but it is still a list
for the sake of consistency.  Hence the sheet is accessed with the list
subsetter `[[1]]`.

```{r}
cells <- contents(path)[[1]]
```

Cell formatting isn't required for this vignette, but if it were, it would be
imported via the `formats()` function.

```{r, eval = FALSE}
formatting <- formats(path)[[1]]
```
...
col_headers <- 
  fixed_price %>%
  offset(cells, rows = -1) %>%  # Offset up one row to "IF NWPL Rocky Mountains"
...
```{r}
datacells %>%
  NNW(col_headers[[1]], "region") %>% # This header isn't in every column
  NNW(col_headers[[2]], "price") %>%  # Nor is this header
  N(col_headers[[3]], "bid_offer")    # But this one is
```
...
tidy <- function(x) {
  ...
  datacells %>%
    NNW(col_headers[[1]], "region") %>%
    NNW(col_headers[[2]], "price") %>%
    N(col_headers[[3]], "bid_offer")
}
```
...
tidy <- function(x) {
  ...
  datacells %>%
    ...
    NNW(col_headers[[2]], "price") %>%
    N(col_headers[[3]], "bid_offer") %>%
    W(row_headers, "context") # This is the only new line
}
