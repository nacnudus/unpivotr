...
converting to a matrix, transposing the matrix, and converting back to a data
frame, but this almost always results in all the data types being converted.
...
the header of a data frame.

```{r}
xlsx_cells(path, sheet = "non-text headers") %>%
  behead(W, name, character) %>%
  behead(N, `academic-year`, date) %>%
...
This is trickier when a single set of headers is of mixed data types, e.g. some
character and some date.  You can deal with this by converting the date values
to text *before* using `behead()`.
