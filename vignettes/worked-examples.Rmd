...
transpose is the same is importing the usual layout, merely using the `W` (west)
direction instead of `N` (north) when beheading the headers.
...
approach would be to match the inner headers to the outer ones by proximity, but
...
There are four functions to resolve this: `ABOVE()`, `LEFT()`, `BELOW()` and
`RIGHT()`, which do the same job as `N()`, `NNW()` etc., but require two sets of
header cells instead of one:

* `header` is the cells whose value will be used as the header
* `corners` is the cells whose position is in one corner of the domain of the
  header (e.g. the top-left-hand corner).
...
first_header_row_corners <-
  filter(all_cells, row == 2, local_format_id %in% left_borders) %>%
  select(row, col)
first_header_row_corners

first_header_col_corners <-
  filter(all_cells, col == 2, local_format_id %in% top_borders) %>%
  select(row, col)
first_header_col_corners
```

The rest of this example is the same as "Two clear rows and columns of text
headers, top-aligned and left-aligned", the only difference being the use of
`ABOVE()` and `LEFT()` instead of `NNW()` and `WNW()`
...
data_cells %>%
  ABOVE(first_header_row, corners = first_header_row_corners) %>%
  N(second_header_row) %>%
  LEFT(first_header_col, corners = first_header_col_corners) %>%
  W(second_header_col) %>%
  select(-row, -col)
```
...
The following table of products and their
...
### Superscript symbols in data cells to reference notes
...
xlsx_cells(path, sheet = "superscript symbols") %>%
  ...
  filter(map_int(character_formatted, nrow) != 1) %>%
  select(row, col, character)
```
...
xlsx_cells(path, sheet = "non-text headers") %>%
  select(row, col, data_type, character, numeric, date) %>%
  behead(N, header, date = ~ strftime(.x, "%Y"), character = toupper)
```
...
* Can't unpivotr just work it out by magic?
  - adversarial example
* Data validation (tidyxl vignette?)
* Other gotchas
